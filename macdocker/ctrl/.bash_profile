...
#============================================================
promptls () {
 mydir="$(ls -FtC1 "$PWD" 2>/dev/null | tr '\n' ' ' | cut -c -256)"
echo -n "$mydir"
}
...
promptgit () {
	if [ -d .git ] ; then
		...
		echo -en "Git Status: $gitstat "
		echo -en " $gitinfo "
		if [ $gitcnt -gt 1 ] ; then
			echo -en ". . . $gitcnt changed files "
		fi
	else
		mydir="$(ls -FtC1 "$PWD" 2>/dev/null | tr '\n' ' ' | cut -c -256)"
		echo -n "$mydir"
...
printf ${ALERT}
	...
	# mark last output
        PS1=${PS1}"\n\[${BYellow}\][\$(checkexit) \$(prompthist) \[${NC}\]\[${BYellow}\]]<=[runtime:\[${BWhite}\]\${clock_show}\[${BYellow}\]]\[${NC}\]"
        # Start with time of day
        PS1=${PS1}"\[${Yellow}\]_______________________[\[${BPurple}\]\@ \d\[${Yellow}\]]\[${NC}\]"
...
#        PS1=${PS1}"\n\[${Yellow}\][\[${BCyan}\]\u\[${NC}\]@\[${BBlack}\]\[${On_Cyan}\]\h\[${Yellow}\]]\[${NC}\]"
	...
	PS1=${PS1}"\n\[${Yellow}\]Dir: [ \[${BBlue}\][\$(promptpwd)\[${Yellow}\] ]"
	PS1=${PS1}"\n\[${Yellow}\]  |_/\[${NC}\]"
	PS1=${PS1}"\[${Yellow}\][ \[${NC}\]\$(promptgit)\[${Yellow}\] ]\[${NC}\]"
	# set the prompt
	PS1=${PS1}"\n\n\[${BYellow}\]=> \[${NC}\]"
